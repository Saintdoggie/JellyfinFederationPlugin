...
- [x] `LocalMediaMountService` owns its rclone process, starts without a shell, verifies an instance-specific marker and restores the mount after restart. Companion downloads or ships rclone and auto-starts the mount after install; install.ps1 installs WinFsp with one UAC prompt.
